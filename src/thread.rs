...
impl<A: Auth> ThreadActor<A> {
    ...
    async fn set_thread_goal_objective(
        ...
        let (goal, previous_status) = if let Some(previous_goal) =
            ...
            let previous_status = ExternalGoalPreviousStatus::from(previous_goal);
            let goal = state_db
                .thread_goals()
                .update_thread_goal(
                    thread_id,
    ...
    fn replay_event_msg(&self, msg: &EventMsg) {
        match msg {
            ...
            EventMsg::ThreadGoalUpdated(event) => {
                self.client.send_thread_goal_updated(
                    &event.thread_id,
                    event.turn_id.clone(),
                    &event.goal,
                );
...
fn state_thread_goal_status_from_protocol(
    ...
    match status {
        ...
        ThreadGoalStatus::Blocked => Ok(codex_state::ThreadGoalStatus::Blocked),
        ThreadGoalStatus::UsageLimited => Ok(codex_state::ThreadGoalStatus::UsageLimited),
        ThreadGoalStatus::BudgetLimited => Ok(codex_state::ThreadGoalStatus::BudgetLimited),
        ThreadGoalStatus::Complete => Ok(codex_state::ThreadGoalStatus::Complete),
    }
}
...
mod tests {
    ...
    async fn test_codex_plan_mode_config_switches_collaboration_mode() -> anyhow::Result<()> {
...

        {
            ...
            assert!(matches!(
                ops.last(),
                Some(Op::ThreadSettings { thread_settings })
                    if thread_settings
                        .collaboration_mode
                        .as_ref()
        ...
        assert!(matches!(
            ...
            Some(Op::ThreadSettings { thread_settings })
                if thread_settings
                    .collaboration_mode
                    .as_ref()
                    .is_some_and(|mode| mode.mode == ModeKind::Default)
        ));
